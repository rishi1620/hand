    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Here is your daily recovery summary.</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2 bg-gradient-to-br from-primary/10 to-transparent border-primary/20">
          <CardHeader>
            <CardTitle className="text-primary flex items-center gap-2">
              <Calendar className="w-5 h-5" /> Today's Session Plan
            </CardTitle>
            <CardDescription>Prescribed by Dr. Emily Chen</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-4 mb-6">
              ...
              <div className="bg-white  p-4 rounded-xl border flex-1 text-center">
                ...
                <div className="text-xs text-muted-foreground uppercase mt-1">Intensity</div>
              </div>
            </div>
            <Button className="w-full sm:w-auto" size="lg" onClick={() => navigate('/patient/live')}>
              <PlayCircle className="w-5 h-5 mr-2" /> Start Now
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                 <Trophy className="w-4 h-4 text-yellow-500" /> Current Streak
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-4xl font-bold">4 <span className="text-lg text-muted-foreground font-normal">days</span></div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                 <Activity className="w-4 h-4 text-emerald-500" /> Compliance Score
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-4xl font-bold">{patientData.complianceScore}%</div>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent>
           <div className="border rounded-lg divide-y">
              {[
                { date: 'Yesterday', duration: '15 min', score: 'Excellent' },
                { date: 'Oct 24', duration: '12 min', score: 'Good' },
              ].map((s, i) => (
                 <div key={i} className="p-4 flex justify-between items-center bg-muted/50 ">
                    <div>
                      <div className="font-medium">{s.date}</div>
                      <div className="text-sm text-muted-foreground">{s.duration} session</div>
                    </div>
